DROP PROCEDURE IF EXISTS Creatures_Get_BattleMonsterPowers;

DELIMITER ;;
CREATE PROCEDURE Creatures_Get_BattleMonsterPowers(
...
BEGIN
    ...
    SELECT b.id, b.monster_power_id, mp.name, b.active, b.active_target_creature_id, b.uses_remaining,
           l.quantity, l.limited_use_type_id, ma.legendary_cost, ma.action_type_id, mp.recharge_min, mp.recharge_max
    FROM battle_monster_powers b
        JOIN monster_powers mp ON mp.id = b.monster_power_id AND mp.monster_power_type_id = powerTypeId
        LEFT JOIN monster_actions ma ON mp.id = ma.monster_power_id
