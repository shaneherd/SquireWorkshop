DROP PROCEDURE IF EXISTS Powers_Get_DamageConfigurationsForCharacteristic;

DELIMITER ;;
CREATE PROCEDURE Powers_Get_DamageConfigurationsForCharacteristic(
...
BEGIN
    SELECT pd.power_id, pd.character_advancement, pd.extra, pd.character_level_id, a.name AS character_level_name, a.sid AS character_level_sid, a.user_id = userId AS character_level_is_author,
    ...
    FROM power_damages pd
        JOIN features f ON f.power_id = pd.power_id
...
# CALL Powers_Get_DamageConfigurationsForCharacteristic(2, 0, 0, 0);