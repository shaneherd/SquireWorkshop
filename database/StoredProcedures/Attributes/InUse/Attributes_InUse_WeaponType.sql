DROP PROCEDURE IF EXISTS Attributes_InUse_WeaponType;

DELIMITER ;;
CREATE PROCEDURE Attributes_InUse_WeaponType(
...
BEGIN
    ...
    SELECT w.item_id, 1
    FROM weapons w
        JOIN items ui ON ui.user_id = userId AND ui.id = w.item_id AND w.weapon_type_id = attributeId;

    INSERT INTO temp_items
    SELECT w.item_id, 1
    FROM weapons w
        JOIN items_shared ui ON ui.user_id = userId AND ui.item_id = w.item_id AND w.weapon_type_id = attributeId;
...
# CALL Attributes_InUse_WeaponType(136, 1);
