DROP PROCEDURE IF EXISTS Attributes_InUse_ArmorType;

DELIMITER ;;
CREATE PROCEDURE Attributes_InUse_ArmorType(
...
BEGIN
    ...
    SELECT a.item_id, 1
    FROM armors a
        JOIN items ui ON ui.user_id = userId AND a.item_id = ui.id AND a.armor_type_id = attributeId;

    INSERT INTO temp_items
    SELECT a.item_id, 1
    FROM armors a
        JOIN items_shared ui ON ui.user_id = userId AND a.item_id = ui.item_id AND a.armor_type_id = attributeId;
...
# CALL Attributes_InUse_ArmorType(40, 1);
